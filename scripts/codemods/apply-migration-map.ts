...
const NORMALIZE_PATH = (p: string): string => { // UPPER_CASE
  const normalized = p.replace(/\\\\\\\\/g, '/');
  return normalized.endsWith('/') ? normalized.slice(0, -1) : normalized;
};

// Remove common extensions
const STRIP_EXTENSION = (p: string): string => { // UPPER_CASE
...
MIGRATION_MAP.forEach(({ from, to }) => { // Use UPPER_CASE
  ...
  if (from.endsWith('/*')) {
    ...
    ABSOLUTE_TO_FROM_MAP[toDir] = fromDir; // Use UPPER_CASE
      ...
      if (fromAbsNoExt !== fromAbs && !ABSOLUTE_FROM_TO_MAP[fromAbsNoExt]) { // Use UPPER_CASE
      ...
      const toAbsNoExt = STRIP_EXTENSION(toAbs); // Use UPPER_CASE func
       if (toAbsNoExt !== toAbs && !ABSOLUTE_TO_FROM_MAP[toAbsNoExt]) { // Use UPPER_CASE
...
const PACKAGE_ALIAS_CONFIG: Record<string, string> = { // UPPER_CASE & Disable rule for keys
...
const FIND_PACKAGE_FOR_NEW_PATH = (newAbsolutePath: string): { pkgRoot: string; alias: string; pathInPkg: string } | null => { // UPPER_CASE
  const normalizedNewPath = NORMALIZE_PATH(newAbsolutePath); // Use UPPER_CASE func
  for (const [pkgDir, alias] of Object.entries(PACKAGE_ALIAS_CONFIG)) { // Use UPPER_CASE
...
export default function transformer(file: FileInfo, api: API, _options: Options) { // Rename options to _options
  ...
  const updateImportSource = (importLiteral: StringLiteral): void => {
      ...
      try {
        ...
        // 2. Handle relative imports './...' or '../...'
            ...
            const oldCurrentAbsolutePath = ABSOLUTE_TO_FROM_MAP[currentNewAbsolutePath] ?? ABSOLUTE_TO_FROM_MAP[STRIP_EXTENSION(currentNewAbsolutePath)]; // Use UPPER_CASE, Use UPPER_CASE func

            if (!oldCurrentAbsolutePath) {
                console.warn(`[WARN] Could not find old path for current file: ${currentNewAbsolutePath}`);
            ...
            if (newAbsoluteImportPath) {
                ...
                console.warn(`[WARN] Could not find new path for relative import: ${originalSource} (resolved old: ${oldAbsoluteImportPath}) in ${currentNewAbsolutePath} (old context: ${oldCurrentAbsolutePath})`);
            }
        }
        ...
        if (newSource !== originalSource) {
          ...
          changed = true;
           ...
           console.error(`[ERROR] Failed to process import \\"${originalSource}\\" in ${currentNewAbsolutePath}: ${errorMessage}`);
      }
  };
  ...
  root.find(j.CallExpression, {
   ...
   .filter(exp => exp.value.source?.type === 'StringLiteral')
   .forEach(exp => {
       if (exp.value.source) { // Type guard
           updateImportSource(exp.value.source as StringLiteral);
       }
   });
