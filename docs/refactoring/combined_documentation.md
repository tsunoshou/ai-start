...
| `domain/repositories/base.repository.interface.ts`            | `packages/shared/domain/repositories/base.repository.interface.ts`                | 共通のベースリポジトリインターフェース。`@core/shared/domain/repositories/base.repository.interface.ts` として配置。内部インポートパス (`@/shared/...`) は `@core/shared/...` 等への変更が必要。                               |
| `domain/models/user/user.entity.ts`                         | `packages/user/domain/models/user.entity.ts`                                      | Userエンティティ。`@core/user/domain/models/user.entity.ts` として配置。内部インポートパスの変更が必要。                                                                                                                      |
| `domain/models/user/user-role.enum.ts`                      | `packages/user/domain/models/user-role.enum.ts`                                   | User Role Enum。`@core/user/domain/models/user-role.enum.ts` として配置。                                                                                                                                     |
| `domain/models/user/user-name.vo.ts`                        | `packages/user/domain/models/user-name.vo.ts`                                     | User Name Value Object。`@core/user/domain/models/user-name.vo.ts` として配置。内部インポートパスの変更が必要。                                                                                                            |
| `domain/models/user/user-id.vo.ts`                          | `packages/user/domain/models/user-id.vo.ts`                                       | User ID Value Object。`@core/user/domain/models/user-id.vo.ts` として配置。内部インポートパスの変更が必要。                                                                                                              |
| `domain/models/user/__tests__/user.entity.test.ts`            | `packages/user/domain/models/__tests__/user.entity.test.ts`                       | Userエンティティのテスト。`*.unit.test.ts` へのリネーム推奨。内部インポートパスの変更が必要。                                                                                                                                       |
| `domain/models/user/__tests__/user-name.vo.test.ts`           | `packages/user/domain/models/__tests__/user-name.vo.test.ts`                      | User Name VOのテスト。`*.unit.test.ts` へのリネーム推奨。内部インポートパスの変更が必要。                                                                                                                                    |
| `domain/models/user/__tests__/user-id.vo.test.ts`             | `packages/user/domain/models/__tests__/user-id.vo.test.ts`                        | User ID VOのテスト。`*.unit.test.ts` へのリネーム推奨。内部インポートパスの変更が必要。                                                                                                                                      |
| `domain/models/base/base.entity.ts`                         | `packages/shared/domain/models/base/base.entity.ts`                               | 共通のベースエンティティ。`@core/shared/domain/models/base/base.entity.ts` として配置。内部インポートパス (`@/shared/...`) は `@core/shared/...` 等への変更が必要。                                                         |
| `domain/services/ai/AIService.ts`                         | `packages/ai/domain/services/ai.service.ts` (または `shared`)                   | AI関連サービス。`@core/ai/domain/services/ai.service.ts` または `@core/shared/domain/services/ai.service.ts` として配置。専用パッケージ `ai` を作成するか、`shared` に含めるか要検討。ファイル名 `ai.service.ts` へ変更推奨。 |
...
| `infrastructure/database/repositories/base.repository.ts`            | `packages/shared/base/infrastructure/repositories/base.repository.ts`                    | 汎用ベースリポジトリ。全ドメインが利用する基盤インフラの抽象定義として `shared/base` 配下へ。                                                                                                                  |
...
| `infrastructure/database/utils/migrate-all.ts`                       | `packages/shared/infrastructure/database/utils/migrate-all.ts`                               | 全環境マイグレーションスクリプト。共通スクリプトとして `shared` へ（あるいはルート `scripts`）。                                                                                                                    |\
| `infrastructure/database/utils/migrate-env.ts`                       | `packages/shared/infrastructure/database/utils/migrate-env.ts`                               | 特定環境マイグレーションスクリプト。共通スクリプトとして `shared` へ（あるいはルート `scripts`）。                                                                                                                    |\
...
| `infrastructure/mappers/base.mapper.ts`                              | `packages/shared/base/infrastructure/mappers/base.mapper.ts`                                      | 汎用ベースマッパー。全ドメインが利用する基盤インフラの抽象定義として `shared/base` 配下へ。                                                                                                                                                 |
| `infrastructure/mappers/user.mapper.ts`                              | `packages/user/infrastructure/mappers/user.mapper.ts`                                        | Userドメイン固有マッパー。`user` パッケージへ。                                                                                                                                               |\
| `infrastructure/mappers/utils/mapping-helpers.ts`                    | `packages/shared/infrastructure/mappers/utils/mapping-helpers.ts`                            | マッピングヘルパー関数。共有インフラとして `shared` へ。                                                                                                                                              |\
| `infrastructure/mappers/__tests__/base.mapper.test.ts`             | `packages/shared/base/infrastructure/mappers/__tests__/base.mapper.test.ts`                         | ベースマッパーテスト。全ドメインが利用する基盤インフラの抽象定義のテストとして `shared/base` 配下へ。                                                                                                                                                 |
...
| `shared/value-objects/base-id.vo.ts`                               | `packages/shared/base/domain/value-objects/base-id.vo.ts`                               | 全ID値オブジェクトの基底クラス。`shared/base` のドメイン層へ。                                          |\
| `shared/value-objects/base.vo.ts`                                  | `packages/shared/base/domain/value-objects/base.vo.ts`                                  | 全値オブジェクトの基底クラス。`shared/base` のドメイン層へ。                                           |\
