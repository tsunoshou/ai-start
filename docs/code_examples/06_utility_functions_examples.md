 ...
 */
export function userDomainToEntity(user: User, preservePassword = false): Omit<UserEntity, 'hashed_password'> & { hashed_password?: string } {
  return {
    id: user.id,
    email: user.email,
 ...
 */
export function projectDomainToResponse(
  project: Project,
  isOwner: boolean
): ProjectResponse {
  return {
...
export function isAiServiceErrorRetryable(error: unknown): boolean {
  ...
  return (
    retryableAiErrors.some(term => errorMessage.includes(term)) ||
    isNetworkErrorRetryable(error)
  );
}
```
